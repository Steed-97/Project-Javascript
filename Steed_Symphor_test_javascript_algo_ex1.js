...
//Fonction pour remplacer valeur de la grille donnée dans tableau vide
function  F11() {
  for (let i = 0; i < to_verify.length; i++) {
      to_verify[i]=array_number[i]
      }
  }

//Fonction pour afficher grille dans HTML
const divResultat = document.querySelector(#resutat)
function F12() {
  var tableau="";
  for (let i=0; i < to_verify.length; i++){
    tableau += "<div>";
    for(let j=0; j < to_verify.length; j++)
    tableau += "</div>";
  }
  divResultat.innerHTML = tableau
}
